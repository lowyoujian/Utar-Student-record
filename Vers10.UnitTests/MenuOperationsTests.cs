...
namespace Vers10.UnitTests
{
    ...
    public class DummyRandomNumberGenerator : IRandomFunctionality
    {
        ...
        public int[] valuesToReturns = { 0, 2, 3, 1000, 3, 55, 3, 65, 3, 75, 3, 60, 3, 70, 3, 80, 1, 0, 2, 2000, 3, 45, 3, 55, 3, 65, 3, 70, 3, 80, 3, 90 };

        public DummyRandomNumberGenerator()
        { Reset(); }

        public DummyRandomNumberGenerator(int[] valuesToReturns)
        { this.valuesToReturns = valuesToReturns; }
    ...
    public class MockDisplay : DisplayFunctionality
    {
        public List<string[]> archiveMessages;
        public MockDisplay()
        {
            archiveMessages = new List<string[]>();
        }

        public void DisplayMessages(List<string> messages)
        {
            archiveMessages.Add(messages.ToArray<string>());
        }
    }


    [TestFixture]
    public class DisplayOperationsTests
    {
        FileOperations fileOperation;
        DisplayOperations displayOperation;
        MockDisplay mockDisplay;
        StudentRecordTests studentRecordTests;
        TrimesterPerformanceTests trimesterPerformanceTests;

        [TestFixtureSetUp]
        public void SetupObjectsForTests()
        {
            fileOperation = new FileOperations();
            studentRecordTests = new StudentRecordTests();
            trimesterPerformanceTests = new TrimesterPerformanceTests();
        }
        ...
        public void SetupForEveryTest()
        {
            ...
            displayOperation = new DisplayOperations(mockDisplay);
        }

        [Test]
        public void ShowSubjectList_NullArgument_EmptyArchive()
        {
            displayOperation.ShowSubjectList(null);
            Assert.AreEqual(0, mockDisplay.archiveMessages.Count);
        }


        [Test]
        public void ShowSubjectList_3NormalEntries()
        {
            //  To test this method, use an existing file in the same 
            // directory as the dll for this class library, and then read it out again using
            // ReadStringsFromFile
            ...
            Assert.AreEqual("*********** SUBJECTS LIST ************", mockDisplay.archiveMessages[0][0]);
            Assert.AreEqual("Total number of subjects in this list is 3", mockDisplay.archiveMessages[0][1]);

            Assert.AreEqual("\nUnit code and name : UECS1111 Subject1", mockDisplay.archiveMessages[1][0]);
            Assert.AreEqual("Prerequisites : ", mockDisplay.archiveMessages[1][1]);
            Assert.AreEqual("Credit hours : 4", mockDisplay.archiveMessages[1][2]);
            Assert.AreEqual("Classification : compulsory", mockDisplay.archiveMessages[1][3]);
            Assert.AreEqual("Trimesters offered : Y1T1 Y2T1 ", mockDisplay.archiveMessages[1][4]);
            Assert.AreEqual("Campuses offered : FES FCI ", mockDisplay.archiveMessages[1][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[1][6]);
            Assert.AreEqual("Coursework component : 40", mockDisplay.archiveMessages[1][7]);

            Assert.AreEqual("\nUnit code and name : UECS2222 Subject2", mockDisplay.archiveMessages[2][0]);
            Assert.AreEqual("Prerequisites : UECS4444 UECS5555 ", mockDisplay.archiveMessages[2][1]);
            Assert.AreEqual("Credit hours : 4", mockDisplay.archiveMessages[2][2]);
            Assert.AreEqual("Classification : elective", mockDisplay.archiveMessages[2][3]);
            Assert.AreEqual("Trimesters offered : Y3T3 Y2T2 ", mockDisplay.archiveMessages[2][4]);
            Assert.AreEqual("Campuses offered : FES ", mockDisplay.archiveMessages[2][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[2][6]);
            Assert.AreEqual("Coursework component : 30", mockDisplay.archiveMessages[2][7]);

            Assert.AreEqual("\nUnit code and name : UECS3333 Subject3", mockDisplay.archiveMessages[3][0]);
            Assert.AreEqual("Prerequisites : UECS2222 ", mockDisplay.archiveMessages[3][1]);
            Assert.AreEqual("Credit hours : 3", mockDisplay.archiveMessages[3][2]);
            Assert.AreEqual("Classification : major", mockDisplay.archiveMessages[3][3]);
            Assert.AreEqual("Trimesters offered : Y1T1 ", mockDisplay.archiveMessages[3][4]);
            Assert.AreEqual("Campuses offered : FICT ", mockDisplay.archiveMessages[3][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[3][6]);
            Assert.AreEqual("Coursework component : 50", mockDisplay.archiveMessages[3][7]);

        }

        [Test]
        public void ShowTrimesterPerformance_NullArgument_EmptyArchive()
        {
            displayOperation.ShowTrimesterPerformance(null);
            Assert.AreEqual(0, mockDisplay.archiveMessages.Count);
        }


        [Test] // For this test, we reuse the test info used in CalculateCGPAOverall_TrimestersWithRepeatedUnits in StudentRecordTests
        public void ShowTrimesterPerformance_3TrimestersIncludeRepeatedUnits()
        {
            List<StudyUnit> studyUnits = studentRecordTests.generateStudyListWithoutPrerequisites();
            StudentRecord studRecord = trimesterPerformanceTests.generateStudentRecordWithTrimestersForCalculation();

            studRecord.CheckForRepeatedUnits();

            studentRecordTests.PerformCalculcationForEntireStudentRecord(studRecord, studyUnits);

            displayOperation.ShowTrimesterPerformance(studRecord.trimesterPerformanceList[0]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[0][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[0][1]);
            Assert.AreEqual("Unit : UECS1101 somename, Mark : 55, GradePoint : 2.33", mockDisplay.archiveMessages[0][2]);
            Assert.AreEqual("Unit : UECS1102 somename, Mark : 68, GradePoint : 3", mockDisplay.archiveMessages[0][3]);
            Assert.AreEqual("Unit : UECS1103 somename, Mark : 82, GradePoint : 4", mockDisplay.archiveMessages[0][4]);
            Assert.AreEqual("GPA : 3.03 CGPA : 3.03", mockDisplay.archiveMessages[0][5]);
            Assert.AreEqual("Credit hours this trimester : 10 Accumulated credit hours : 10", mockDisplay.archiveMessages[0][6]);


            displayOperation.ShowTrimesterPerformance(studRecord.trimesterPerformanceList[1]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[1][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[1][1]);
            Assert.AreEqual("(*1) Unit : UECS1201 somename, Mark : 51, GradePoint : 2", mockDisplay.archiveMessages[1][2]);
            Assert.AreEqual("(*1) Unit : UECS1202 somename, Mark : 40, GradePoint : 0", mockDisplay.archiveMessages[1][3]);
            Assert.AreEqual("Unit : UECS1203 somename, Mark : 73, GradePoint : 3.33", mockDisplay.archiveMessages[1][4]);
            Assert.AreEqual("GPA : 3.33 CGPA : 3.12", mockDisplay.archiveMessages[1][5]);
            Assert.AreEqual("Credit hours this trimester : 4 Accumulated credit hours : 14", mockDisplay.archiveMessages[1][6]);


            displayOperation.ShowTrimesterPerformance(studRecord.trimesterPerformanceList[2]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[2][0]);
            Assert.AreEqual("Year : 2014 Month : OCT Trimester : Y1T3", mockDisplay.archiveMessages[2][1]);
            Assert.AreEqual("Unit : UECS1301 somename, Mark : 78, GradePoint : 3.67", mockDisplay.archiveMessages[2][2]);
            Assert.AreEqual("Unit : UECS1302 somename, Mark : 92, GradePoint : 4", mockDisplay.archiveMessages[2][3]);
            Assert.AreEqual("(*2) Unit : UECS1201 somename, Mark : 63, GradePoint : 2.67", mockDisplay.archiveMessages[2][4]);
            Assert.AreEqual("(*2) Unit : UECS1202 somename, Mark : 52, GradePoint : 2", mockDisplay.archiveMessages[2][5]);
            Assert.AreEqual("GPA : 3.47 CGPA : 3.26", mockDisplay.archiveMessages[2][6]);
            Assert.AreEqual("Credit hours this trimester : 10 Accumulated credit hours : 24", mockDisplay.archiveMessages[2][7]);


            displayOperation.ShowTrimesterPerformance(studRecord.trimesterPerformanceList[3]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[3][0]);
            Assert.AreEqual("Year : 2015 Month : JAN Trimester : Y2T1", mockDisplay.archiveMessages[3][1]);
            Assert.AreEqual("Unit : UECS2101 somename, Mark : 73, GradePoint : 3.33", mockDisplay.archiveMessages[3][2]);
            Assert.AreEqual("Unit : UECS2102 somename, Mark : 79, GradePoint : 3.67", mockDisplay.archiveMessages[3][3]);
            Assert.AreEqual("(*3) Unit : UECS1202 somename, Mark : 65, GradePoint : 3", mockDisplay.archiveMessages[3][4]);
            Assert.AreEqual("GPA : 3.33 CGPA : 3.29", mockDisplay.archiveMessages[3][5]);
            Assert.AreEqual("Credit hours this trimester : 11 Accumulated credit hours : 35", mockDisplay.archiveMessages[3][6]);

        }

        [Test]
        public void ShowStudentRecord_NullArgument_EmptyArchive()
        {
            displayOperation.ShowTrimesterPerformance(null);
            Assert.AreEqual(0, mockDisplay.archiveMessages.Count);
        }
        ...
        [Test] // For this test, we reuse the test info used in CalculateCGPAOverall_TrimestersWithRepeatedUnits in StudentRecordTests
        public void ShowStudentRecord_UseDataFromExistingTests()
        {
            List<StudyUnit> studyUnits = studentRecordTests.generateStudyListWithoutPrerequisites();
            StudentRecord studRecord = trimesterPerformanceTests.generateStudentRecordWithTrimestersForCalculation();
            ...
            displayOperation.ShowStudentRecord(studRecord);

            Assert.AreEqual("Name : ahmad ID Number : 12345 Gender : male", mockDisplay.archiveMessages[0][0]);

            Assert.AreEqual("Email Address : ahmad@gmail.com Contact Number : 012-44444", mockDisplay.archiveMessages[0][1]);

            Assert.AreEqual("Programme : SE Total credit hours : 120", mockDisplay.archiveMessages[0][2]);
            Assert.AreEqual("Academic Status : normal", mockDisplay.archiveMessages[0][3]);

            // No need to check for trimesters in this student record because the data here is 
            // identical to that used in the test ShowTrimesterPerformance_3TrimestersIncludeRepeatedUnits
        }


        [Test]
        public void ShowUnitStatsForTrimester_NullArgument_EmptyArchive()
        {
            displayOperation.ShowUnitStatsForTrimester(null);
            Assert.AreEqual(0, mockDisplay.archiveMessages.Count);
        }


        [Test] // Reuse the test data used in CalculateOverallResults_CombinationOfDifferentMarks
        // in GradingSchemeResultsTests
        public void ShowUnitStatsForTrimester_UseDataFromExistingTest()
        {
            GradingSchemeResults gradeResults = new GradingSchemeResults();
            gradeResults.AddStudentMark("Muthu", "1111", 85);
            gradeResults.AddStudentMark("Ali", "2222", 32);
            gradeResults.AddStudentMark("Chong", "3333", 55);
            gradeResults.AddStudentMark("Sammy", "4444", 67);
            gradeResults.AddStudentMark("John", "5555", 77);
            gradeResults.AddStudentMark("Ahmad", "6666", 89);
            gradeResults.AddStudentMark("Wendy", "7777", 58);
            gradeResults.AddStudentMark("Peter", "8888", 57);

            gradeResults.CalculateOverallResults();

            displayOperation.ShowUnitStatsForTrimester(gradeResults);

            Assert.AreEqual("0 - 49 : 1 students, Percentage of total : 12.5, 1: 2222 Ali ", mockDisplay.archiveMessages[0][0]);
            Assert.AreEqual("50 - 54 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[0][1]);
            Assert.AreEqual("55 - 59 : 3 students, Percentage of total : 37.5, 1: 3333 Chong 2: 7777 Wendy 3: 8888 Peter ", mockDisplay.archiveMessages[0][2]);
            Assert.AreEqual("60 - 64 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[0][3]);
            Assert.AreEqual("65 - 69 : 1 students, Percentage of total : 12.5, 1: 4444 Sammy ", mockDisplay.archiveMessages[0][4]);
            Assert.AreEqual("70 - 74 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[0][5]);
            Assert.AreEqual("75 - 79 : 1 students, Percentage of total : 12.5, 1: 5555 John ", mockDisplay.archiveMessages[0][6]);
            Assert.AreEqual("80 - 89 : 2 students, Percentage of total : 25, 1: 1111 Muthu 2: 6666 Ahmad ", mockDisplay.archiveMessages[0][7]);
            Assert.AreEqual("90 - 100 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[0][8]);
    ...
    public class SimulateKeyboardInput : InputFunctionality
    {

        public string[] valuesToReturn;
        private int count = 0;

        public SimulateKeyboardInput(string[] valuesToReturn)
        {
    ...
    [TestFixture]
    public class MenuOperationsTests
    {
        ...
        {
            menuOperation = new MenuOperations();
            mockDisplay = new MockDisplay();
        }

        // Both the two methods LoadSubjectList and ShowSubjectList are tested together in 
        // a single test method as ShowSubjectList requires LoadSubjectList to run first.
        // This is therefore a form of integration testing
        [Test]
        public void ShowSubjectList_AND_LoadSubjectList_UseNormalFile()
        {

            string[] simulatedValuesToReturn = new string[1];

            simulatedValuesToReturn[0] = "basicsubjectlist"; // filename containing subject list to be loaded
            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.LoadSubjectList();

            Assert.AreEqual("List of study units loaded successfully and added to existing list !", mockDisplay.archiveMessages[0][0]);

            menuOperation.displayOperation = new DisplayOperations(mockDisplay);

            menuOperation.ShowSubjectList();

            Assert.AreEqual("*********** SUBJECTS LIST ************", mockDisplay.archiveMessages[1][0]);
            Assert.AreEqual("Total number of subjects in this list is 5", mockDisplay.archiveMessages[1][1]);

            Assert.AreEqual("\nUnit code and name : UECS1111 Subject1", mockDisplay.archiveMessages[2][0]);
            Assert.AreEqual("Prerequisites : ", mockDisplay.archiveMessages[2][1]);
            Assert.AreEqual("Credit hours : 4", mockDisplay.archiveMessages[2][2]);
            Assert.AreEqual("Classification : compulsory", mockDisplay.archiveMessages[2][3]);
            Assert.AreEqual("Trimesters offered : Y1T1 Y2T1 ", mockDisplay.archiveMessages[2][4]);
            Assert.AreEqual("Campuses offered : FES FCI ", mockDisplay.archiveMessages[2][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[2][6]);
            Assert.AreEqual("Coursework component : 40", mockDisplay.archiveMessages[2][7]);

            Assert.AreEqual("\nUnit code and name : UECS2222 Subject2", mockDisplay.archiveMessages[3][0]);
            Assert.AreEqual("Prerequisites : ", mockDisplay.archiveMessages[3][1]);
            Assert.AreEqual("Credit hours : 4", mockDisplay.archiveMessages[3][2]);
            Assert.AreEqual("Classification : elective", mockDisplay.archiveMessages[3][3]);
            Assert.AreEqual("Trimesters offered : Y3T3 Y2T2 ", mockDisplay.archiveMessages[3][4]);
            Assert.AreEqual("Campuses offered : FES ", mockDisplay.archiveMessages[3][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[3][6]);
            Assert.AreEqual("Coursework component : 30", mockDisplay.archiveMessages[3][7]);

            Assert.AreEqual("\nUnit code and name : UECS3333 Subject3", mockDisplay.archiveMessages[4][0]);
            Assert.AreEqual("Prerequisites : ", mockDisplay.archiveMessages[4][1]);
            Assert.AreEqual("Credit hours : 3", mockDisplay.archiveMessages[4][2]);
            Assert.AreEqual("Classification : major", mockDisplay.archiveMessages[4][3]);
            Assert.AreEqual("Trimesters offered : Y1T1 ", mockDisplay.archiveMessages[4][4]);
            Assert.AreEqual("Campuses offered : FICT ", mockDisplay.archiveMessages[4][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[4][6]);
            Assert.AreEqual("Coursework component : 50", mockDisplay.archiveMessages[4][7]);

            Assert.AreEqual("\nUnit code and name : UECS4444 Subject4", mockDisplay.archiveMessages[5][0]);
            Assert.AreEqual("Prerequisites : UECS1111 ", mockDisplay.archiveMessages[5][1]);
            Assert.AreEqual("Credit hours : 3", mockDisplay.archiveMessages[5][2]);
            Assert.AreEqual("Classification : uniunit", mockDisplay.archiveMessages[5][3]);
            Assert.AreEqual("Trimesters offered : Y2T3 Y3T3 ", mockDisplay.archiveMessages[5][4]);
            Assert.AreEqual("Campuses offered : FES ", mockDisplay.archiveMessages[5][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[5][6]);
            Assert.AreEqual("Coursework component : 70", mockDisplay.archiveMessages[5][7]);

            Assert.AreEqual("\nUnit code and name : UECS5555 Subject5", mockDisplay.archiveMessages[6][0]);
            Assert.AreEqual("Prerequisites : UECS2222 UECS3333 ", mockDisplay.archiveMessages[6][1]);
            Assert.AreEqual("Credit hours : 4", mockDisplay.archiveMessages[6][2]);
            Assert.AreEqual("Classification : majorelective", mockDisplay.archiveMessages[6][3]);
            Assert.AreEqual("Trimesters offered : Y2T1 ", mockDisplay.archiveMessages[6][4]);
            Assert.AreEqual("Campuses offered : FCI ", mockDisplay.archiveMessages[6][5]);
            Assert.AreEqual("Programmes offering this unit : SE ", mockDisplay.archiveMessages[6][6]);
            Assert.AreEqual("Coursework component : 60", mockDisplay.archiveMessages[6][7]);
        }

        // Both the two methods LoadSingleStudentRecord and DisplayStudentRecords are 
        // tested together in a single test method as DisplayStudentRecords requires LoadSingleStudentRecord to run first.
        // This is therefore a form of integration testing
        // The subject list is loaded first before these 2 methods are run.
        [Test]
        public void LoadSubjectList_AND_LoadSingleStudentRecord_AND_DisplayStudentRecords()
        {
            string[] simulatedValuesToReturn = new string[1];
            ...
            simulatedValuesToReturn[0] = "studentrecord1"; // name of file to load single student record from
            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.LoadSingleStudentRecord();

            Assert.AreEqual("Student record loaded successfully and added to existing student record list !", mockDisplay.archiveMessages[1][0]);

            simulatedValuesToReturn[0] = "1 1"; // Display records 1 to 1

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.DisplayStudentRecords();

            Assert.AreEqual("\nStudent Record # 1", mockDisplay.archiveMessages[2][0]);

            Assert.AreEqual("Name : LARRY EDWARDS ID Number : SE1111 Gender : male", mockDisplay.archiveMessages[3][0]);

            Assert.AreEqual("Email Address : LARRY EDWARDS@gmail.com Contact Number : 1234567890", mockDisplay.archiveMessages[3][1]);

            Assert.AreEqual("Programme : SE Total credit hours : 120", mockDisplay.archiveMessages[3][2]);
            Assert.AreEqual("Academic Status : normal", mockDisplay.archiveMessages[3][3]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[4][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[4][1]);
            Assert.AreEqual("Unit : UECS1111 Subject1, Mark : 87, GradePoint : 4", mockDisplay.archiveMessages[4][2]);
            Assert.AreEqual("Unit : UECS2222 Subject2, Mark : 56, GradePoint : 2.33", mockDisplay.archiveMessages[4][3]);
            Assert.AreEqual("Unit : UECS3333 Subject3, Mark : 68, GradePoint : 3", mockDisplay.archiveMessages[4][4]);
            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[4][5]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[4][6]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[5][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[5][1]);
            Assert.AreEqual("Unit : UECS4444 Subject4, Mark : 79, GradePoint : 3.67", mockDisplay.archiveMessages[5][2]);
            Assert.AreEqual("Unit : UECS5555 Subject5, Mark : 62, GradePoint : 2.67", mockDisplay.archiveMessages[5][3]);
            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[5][4]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[5][5]);
        }



        // This is a base integration test involving the following sequence:
        // LoadSubjectList, followed by LoadMultipleStudentRecords (studentrecord1 - studentrecord4)
        // then VerifyStudentRecords and CalculateGradesForStudentRecords for studentrecord1 - 4
        // The method sequence used in this test is reused in different subsequent tests
        // so it is defined here as an ordinary method, and not marked with the [Test] attribute

        public void LoadSubjectList_AND_LoadMultipleStudentRecords_AND_VerifyStudentRecords_AND_CalculateGradesForStudentRecords()
        {
            string[] simulatedValuesToReturn = new string[3];
            ...
            simulatedValuesToReturn[0] = "studentrecord"; // prefix for student record file name
            simulatedValuesToReturn[1] = "1 4"; // range -> studentrecord1 - studentrecord4

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.LoadMultipleStudentRecords();

            Assert.AreEqual("All files have been loaded successfully and added to the existing student record list", mockDisplay.archiveMessages[1][0]);


            simulatedValuesToReturn[0] = "1 4"; // Verify records 1 - 4
            simulatedValuesToReturn[1] = "";

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.VerifyStudentRecords();

            Assert.AreEqual("Verifying Student Record # 1", mockDisplay.archiveMessages[2][0]);
            Assert.AreEqual("Verifying Student Record # 2", mockDisplay.archiveMessages[3][0]);
            Assert.AreEqual("Verifying Student Record # 3", mockDisplay.archiveMessages[4][0]);
            Assert.AreEqual("Verifying Student Record # 4", mockDisplay.archiveMessages[5][0]);

            simulatedValuesToReturn[0] = "1 4"; // Calculate GRades for records 1 - 4
            simulatedValuesToReturn[1] = "";

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.CalculateGradesForStudentRecords();

            Assert.AreEqual("Calculating Grades for Student Record # 1", mockDisplay.archiveMessages[6][0]);
            Assert.AreEqual("Calculating Grades for Student Record # 2", mockDisplay.archiveMessages[7][0]);
            Assert.AreEqual("Calculating Grades for Student Record # 3", mockDisplay.archiveMessages[8][0]);
            Assert.AreEqual("Calculating Grades for Student Record # 4", mockDisplay.archiveMessages[9][0]);
        }
        ...
        // This is a complex integration test involving the following sequence:
        // LoadSubjectList, followed by LoadMultipleStudentRecords (studentrecord1 - studentrecord4)
        // then VerifyStudentRecords and CalculateGradesForStudentRecords for studentrecord1 - 4
        // then DisplayStudentRecords for studentrecord3 - 4

        [Test]
        public void DoBasicSequence_DisplayStudentRecords()
        {
            LoadSubjectList_AND_LoadMultipleStudentRecords_AND_VerifyStudentRecords_AND_CalculateGradesForStudentRecords();

            string[] simulatedValuesToReturn = new string[3];

            simulatedValuesToReturn[0] = "3 4"; // show record # 3 and # 4 only
            simulatedValuesToReturn[1] = "";
            ...
            Assert.AreEqual("\nStudent Record # 3", mockDisplay.archiveMessages[10][0]);

            Assert.AreEqual("Name : GEORGE LIM ID Number : SE3333 Gender : male", mockDisplay.archiveMessages[11][0]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[12][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[12][1]);
            Assert.AreEqual("Unit : UECS1111 Subject1, Mark : 67, GradePoint : 3", mockDisplay.archiveMessages[12][2]);
            Assert.AreEqual("Unit : UECS2222 Subject2, Mark : 74, GradePoint : 3.33", mockDisplay.archiveMessages[12][3]);
            Assert.AreEqual("Unit : UECS3333 Subject3, Mark : 59, GradePoint : 2.33", mockDisplay.archiveMessages[12][4]);
            Assert.AreEqual("GPA : 2.94 CGPA : 2.94", mockDisplay.archiveMessages[12][5]);
            Assert.AreEqual("Credit hours this trimester : 11 Accumulated credit hours : 11", mockDisplay.archiveMessages[12][6]);


            Assert.AreEqual(" ", mockDisplay.archiveMessages[13][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[13][1]);
            Assert.AreEqual("Unit : UECS4444 Subject4, Mark : 90, GradePoint : 4", mockDisplay.archiveMessages[13][2]);
            Assert.AreEqual("Unit : UECS5555 Subject5, Mark : 52, GradePoint : 2", mockDisplay.archiveMessages[13][3]);
            Assert.AreEqual("GPA : 2.86 CGPA : 2.91", mockDisplay.archiveMessages[13][4]);
            Assert.AreEqual("Credit hours this trimester : 7 Accumulated credit hours : 18", mockDisplay.archiveMessages[13][5]);


            Assert.AreEqual("\nStudent Record # 4", mockDisplay.archiveMessages[14][0]);
            Assert.AreEqual("Name : JENNY WONG ID Number : SE4444 Gender : female", mockDisplay.archiveMessages[15][0]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[16][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[16][1]);
            Assert.AreEqual("Unit : UECS1111 Subject1, Mark : 51, GradePoint : 2", mockDisplay.archiveMessages[16][2]);
            Assert.AreEqual("Unit : UECS2222 Subject2, Mark : 91, GradePoint : 4", mockDisplay.archiveMessages[16][3]);
            Assert.AreEqual("Unit : UECS3333 Subject3, Mark : 72, GradePoint : 3.33", mockDisplay.archiveMessages[16][4]);
            Assert.AreEqual("GPA : 3.09 CGPA : 3.09", mockDisplay.archiveMessages[16][5]);
            Assert.AreEqual("Credit hours this trimester : 11 Accumulated credit hours : 11", mockDisplay.archiveMessages[16][6]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[17][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[17][1]);
            Assert.AreEqual("Unit : UECS4444 Subject4, Mark : 65, GradePoint : 3", mockDisplay.archiveMessages[17][2]);
            Assert.AreEqual("Unit : UECS5555 Subject5, Mark : 59, GradePoint : 2.33", mockDisplay.archiveMessages[17][3]);
            Assert.AreEqual("GPA : 2.62 CGPA : 2.91", mockDisplay.archiveMessages[17][4]);
            Assert.AreEqual("Credit hours this trimester : 7 Accumulated credit hours : 18", mockDisplay.archiveMessages[17][5]);
        }
        ...
        // LoadSubjectList, followed by LoadMultipleStudentRecords (studentrecord1 - studentrecord4)
        // then VerifyStudentRecords and CalculateGradesForStudentRecords for studentrecord1 - 4
        // then CalculateUnitStats for studentrecord2 - 4

        [Test]
        public void DoBasicSequence_CalculateUnitStats()
        {
            LoadSubjectList_AND_LoadMultipleStudentRecords_AND_VerifyStudentRecords_AND_CalculateGradesForStudentRecords();

            string[] simulatedValuesToReturn = new string[3];

            simulatedValuesToReturn[0] = "2014 JAN Y1T1"; // get stats for this trimester
            simulatedValuesToReturn[1] = "UECS3333"; // get stats for this unit code
            simulatedValuesToReturn[2] = "2 4"; // get stats for this record range

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.CalculateUnitStats();

            Assert.AreEqual("\nStatistics for unit UECS3333 for 2014 JAN Y1T1", mockDisplay.archiveMessages[10][0]);
            Assert.AreEqual("Statistics for record #2 to #4", mockDisplay.archiveMessages[10][1]);

            Assert.AreEqual("0 - 49 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][0]);
            Assert.AreEqual("50 - 54 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][1]);
            Assert.AreEqual("55 - 59 : 1 students, Percentage of total : 33.33, 1: SE3333 GEORGE LIM ", mockDisplay.archiveMessages[11][2]);
            Assert.AreEqual("60 - 64 : 1 students, Percentage of total : 33.33, 1: SE2222 WENDY LIEW ", mockDisplay.archiveMessages[11][3]);
            Assert.AreEqual("65 - 69 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][4]);
            Assert.AreEqual("70 - 74 : 1 students, Percentage of total : 33.33, 1: SE4444 JENNY WONG ", mockDisplay.archiveMessages[11][5]);
            Assert.AreEqual("75 - 79 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][6]);
            Assert.AreEqual("80 - 89 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][7]);
            Assert.AreEqual("90 - 100 : 0 students, Percentage of total : 0, ", mockDisplay.archiveMessages[11][8]);
            Assert.AreEqual("Average mark : 64.33", mockDisplay.archiveMessages[11][9]);

        }
        
        
        // This test for GenerateRecordsFromTemplate uses the set of test data used earlier 
        // in GenerateStudentRecords_ZeroAndTwoSampleRecords but stores this data 
        // (male names, female names, surnames, studentrecord template) on files instead
        // The Assert Equals here is testing the same dummy generated data from 
        // GenerateStudentRecords_ZeroAndTwoSampleRecords.
        // The method sequence used in this test is reused in different subsequent tests
        // so it is defined here as an ordinary method, and not marked with the [Test] attribute

        public void LoadSubjectList_GenerateRecordsFromTemplate()
        {

            string[] simulatedValuesToReturn = new string[4];
            ...
            simulatedValuesToReturn[0] = "malenames femalenames surnames"; // file names for various names
            simulatedValuesToReturn[1] = "2"; // number of records to generate
            simulatedValuesToReturn[2] = "recordtemplate1"; // student file record template for generation
            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));
            menuOperation.dataGenerator = new TestDataGenerator(new DummyRandomNumberGenerator());

            menuOperation.GenerateRecordsFromTemplate();

            Assert.AreEqual("Test records generated successfully and added to existing student record list", mockDisplay.archiveMessages[1][0]);

        }


        // This is a complex integration test involving the following sequence:
        // LoadSubjectList, followed by GenerateRecordsFromTemplate (studentrecord1 - studentrecord2)
        // then DisplayStudentRecords for studentrecord1 - 2 that was just generated.

        [Test]
        public void DoGenerateRandomRecords_DisplayStudentRecords()
        {
            LoadSubjectList_GenerateRecordsFromTemplate();

            string[] simulatedValuesToReturn = new string[4];

            simulatedValuesToReturn[0] = "1 2"; // show record # 1 and # 2 
            simulatedValuesToReturn[1] = "";
            ...
            Assert.AreEqual("Name : Roger Lee ID Number : SE10000 Gender : male", mockDisplay.archiveMessages[3][0]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[4][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[4][1]);
            Assert.AreEqual("Unit : UECS1101 somename, Mark : 55, GradePoint : 2.33", mockDisplay.archiveMessages[4][2]);
            Assert.AreEqual("Unit : UECS1102 somename, Mark : 65, GradePoint : 3", mockDisplay.archiveMessages[4][3]);
            Assert.AreEqual("Unit : UECS1103 somename, Mark : 75, GradePoint : 3.67", mockDisplay.archiveMessages[4][4]);
            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[4][5]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[4][6]);
            ...
            Assert.AreEqual(" ", mockDisplay.archiveMessages[5][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[5][1]);
            Assert.AreEqual("Unit : UECS1201 somename, Mark : 60, GradePoint : 2.67", mockDisplay.archiveMessages[5][2]);
            Assert.AreEqual("Unit : UECS1202 somename, Mark : 70, GradePoint : 3.33", mockDisplay.archiveMessages[5][3]);
            Assert.AreEqual("Unit : UECS1203 somename, Mark : 80, GradePoint : 4", mockDisplay.archiveMessages[5][4]);

            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[5][5]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[5][6]);


            Assert.AreEqual("\nStudent Record # 2", mockDisplay.archiveMessages[6][0]);

            Assert.AreEqual("Name : Wendy Lim ID Number : SE20001 Gender : female", mockDisplay.archiveMessages[7][0]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[8][0]);
            Assert.AreEqual("Year : 2014 Month : JAN Trimester : Y1T1", mockDisplay.archiveMessages[8][1]);
            Assert.AreEqual("Unit : UECS1101 somename, Mark : 45, GradePoint : 0", mockDisplay.archiveMessages[8][2]);
            Assert.AreEqual("Unit : UECS1102 somename, Mark : 55, GradePoint : 2.33", mockDisplay.archiveMessages[8][3]);
            Assert.AreEqual("Unit : UECS1103 somename, Mark : 65, GradePoint : 3", mockDisplay.archiveMessages[8][4]);
            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[8][5]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[8][6]);

            Assert.AreEqual(" ", mockDisplay.archiveMessages[9][0]);
            Assert.AreEqual("Year : 2014 Month : MAY Trimester : Y1T2", mockDisplay.archiveMessages[9][1]);
            Assert.AreEqual("Unit : UECS1201 somename, Mark : 70, GradePoint : 3.33", mockDisplay.archiveMessages[9][2]);
            Assert.AreEqual("Unit : UECS1202 somename, Mark : 80, GradePoint : 4", mockDisplay.archiveMessages[9][3]);
            Assert.AreEqual("Unit : UECS1203 somename, Mark : 90, GradePoint : 4", mockDisplay.archiveMessages[9][4]);

            Assert.AreEqual("GPA : 0.00 CGPA : 0.00", mockDisplay.archiveMessages[9][5]);
            Assert.AreEqual("Credit hours this trimester : 0 Accumulated credit hours : 0", mockDisplay.archiveMessages[9][6]);
        ...
        // This is a complex integration test involving the following sequence:
        // LoadSubjectList, followed by GenerateRecordsFromTemplate (studentrecord1 - studentrecord2)
        // then SaveCurrentRecords (as studentrecord3,4) and LoadMutipleStudentRecords 
        // and finally DisplayStudentRecords for studentrecord1 - 4.


        [Test]
        public void DoGenerateRandomRecords_SaveCurrentRecords_LoadMultipleStudentRecords_DisplayStudentRecords()
        {
            LoadSubjectList_GenerateRecordsFromTemplate();

            string[] simulatedValuesToReturn = new string[4];

            // Save all the records to file first
            simulatedValuesToReturn[0] = "saverecord"; // prefix name of file to save to
            simulatedValuesToReturn[1] = "3"; // start saving with saverecord3, saverecord4, etc ....

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.SaveCurrentRecords();

            Assert.AreEqual("All student records in memory successfully saved to files on disk", mockDisplay.archiveMessages[2][0]);

            // Then reload the saved records, resulting in 4 records in memory

            simulatedValuesToReturn[0] = "saverecord"; // prefix for student record file name
            simulatedValuesToReturn[1] = "3 4"; // range -> saverecord3, saverecord4

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.LoadMultipleStudentRecords();

            Assert.AreEqual("All files have been loaded successfully and added to the existing student record list", mockDisplay.archiveMessages[3][0]);

            simulatedValuesToReturn[0] = "1 4"; // show record # 1 - # 4
            simulatedValuesToReturn[1] = "";
            ...
            Assert.AreEqual("\nStudent Record # 1", mockDisplay.archiveMessages[4][0]);
            Assert.AreEqual("Name : Roger Lee ID Number : SE10000 Gender : male", mockDisplay.archiveMessages[5][0]);

            Assert.AreEqual("\nStudent Record # 2", mockDisplay.archiveMessages[8][0]);
            Assert.AreEqual("Name : Wendy Lim ID Number : SE20001 Gender : female", mockDisplay.archiveMessages[9][0]);

            // #3, #4 are duplicates of #1, #2 

            Assert.AreEqual("\nStudent Record # 3", mockDisplay.archiveMessages[12][0]);
            Assert.AreEqual("Name : Roger Lee ID Number : SE10000 Gender : male", mockDisplay.archiveMessages[5][0]);

            Assert.AreEqual("\nStudent Record # 4", mockDisplay.archiveMessages[16][0]);
            Assert.AreEqual("Name : Wendy Lim ID Number : SE20001 Gender : female", mockDisplay.archiveMessages[9][0]);
        ...
        // LoadSubjectList, followed by LoadMultipleStudentRecords (studentrecord1 - studentrecord4)
        // then VerifyStudentRecords and CalculateGradesForStudentRecords for studentrecord1 - 4
        // then SortStudentRecords for studentrecord1 - 4.

        [Test]
        public void DoBasicSequence_SortStudentRecords()
        {
            LoadSubjectList_AND_LoadMultipleStudentRecords_AND_VerifyStudentRecords_AND_CalculateGradesForStudentRecords();

            string[] simulatedValuesToReturn = new string[3];

            simulatedValuesToReturn[0] = "1 4"; // sort records # 1 and # 4 only
            simulatedValuesToReturn[1] = "2014 MAY Y1T2"; // sort based on final results for this trimester
            simulatedValuesToReturn[2] = "2"; // sort based on GPA

            menuOperation.displayOperation = new DisplayOperations(mockDisplay, new SimulateKeyboardInput(simulatedValuesToReturn));

            menuOperation.SortStudentRecords();

            // Verify that the records are sorted in correct order

            Assert.AreEqual("\nSorted Student Records #1 - #4 for 2014 MAY Y1T2", mockDisplay.archiveMessages[10][0]);

            Assert.AreEqual("\n1. Name : WENDY LIEW ID Number : SE2222 GPA : 3.81 CGPA : 3.70", mockDisplay.archiveMessages[11][0]);

            Assert.AreEqual("\n2. Name : LARRY EDWARDS ID Number : SE1111 GPA : 3.10 CGPA : 3.11", mockDisplay.archiveMessages[12][0]);

            Assert.AreEqual("\n3. Name : GEORGE LIM ID Number : SE3333 GPA : 2.86 CGPA : 2.91", mockDisplay.archiveMessages[13][0]);

            Assert.AreEqual("\n4. Name : JENNY WONG ID Number : SE4444 GPA : 2.62 CGPA : 2.91", mockDisplay.archiveMessages[14][0]);
